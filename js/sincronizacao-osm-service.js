/**
 * Serviço de sincronização com o OpenStreetMap
 * 
 * Este serviço gerencia a sincronização de estabelecimentos aprovados
 * com o OpenStreetMap através da API de integração.
 */
...
class SincronizacaoOSMService {
    constructor() {
        ...
        // Configurar URL do proxy HTTPS
        this.proxyBaseUrl = 'https://dejaljnp.manus.space/proxy';
        
        // Detectar automaticamente a API através do proxy
        this.apiBaseUrl = localStorage.getItem('apiBaseUrl') || this.proxyBaseUrl;
        
        // Adicionar log de inicialização
        this.addLog('info', 'Serviço de sincronização inicializado');
    ...
    async checkApiAvailability() {
        try {
            console.log(`Verificando disponibilidade da API através do proxy: ${this.apiBaseUrl}/health`);
            this.addLog('info', `Verificando disponibilidade da API através do proxy: ${this.apiBaseUrl}/health`);
            ...
            if (response.ok) {
                ...
                this.addLog('success', 'API de integração está disponível através do proxy HTTPS');
            ...
            this.addLog('info', 'Dica: Certifique-se de que a API está rodando em seu computador e o proxy HTTPS está configurado corretamente.');
    ...
    async addEstablishmentToOSM(establishment) {
        try {
            ...
            // Enviar para a API através do proxy
